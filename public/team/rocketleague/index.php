<?php require_once("../../includes/header.php") ?>

<section id="header">
    <div class="jumbotron team-header rocket-league">
        <div class="shadow d-flex align-items-end">
            <div class="container">
                <!-- <div class="d-flex justify-content-center logo-heading">
                    <img class="game-logo " src="/asset/img/fifa/logo/logo.png" alt="logo fifa">
                </div> -->
                <div class="d-flex justify-content-center" style="height: 100px;">
                    <div class="position-relative">
                        <h1 class="font thrill-white h5 mb-4">Notre équipe rocket league</h1>
                        <div class="arrow">
                            <a href="/team/rocketleague/#team">
                                <img src="/asset/img/icon/64/arrow-white.png" alt="flèche vers le bas">
                            </a>
                        </div>
                    </div>
                </div>
            </div>
            <!-- /.container -->
        </div>
        <!-- /.shadow -->
    </div>
    <!-- /.jumbotron -->
</section>
<section id="team" class="thrill-bg-white">
    <div class="container py-5">
        <h3 class="display-4 text-uppercase font-weight-bold thrill-blue">
            roster rocket league
        </h3>
    </div>
    <div class="container py-5">
        <div class="row">
            <div class="col-md-4 mb-5">
                <div class="card-player">
                    <div class="pseudo">
                        <h3 class="font h4 thrill-white">Spectro</h3>
                    </div>
                    <div class="name">
                        <p class="thrill-gray">Pinard Erwan</p>
                    </div>
                    <div class="role">
                        <div class="line-left"></div>
                        <h4 class="font h6">joueur</h4>
                        <div class="line-bottom"></div>
                    </div>
                    <div class="social">
                        <a href="https://twitter.com/Thrill_Spectro">
                            <img src="/asset/img/icon/Twitter/white.png" alt="icon twitter">
                        </a>
                    </div>
                    <div class="player-item">
                        <img src="/asset/img/rocket-league/skins/car-1.png" alt="">
                    </div>
                    <div class="line-item"></div>
                </div>
            </div>
            <!-- /.col -->
            <div class="col-md-4 mb-5">
                <div class="card-player">
                    <div class="pseudo">
                        <h3 class="font h4 thrill-white">Teddy</h3>
                    </div>
                    <div class="name">
                        <p class="thrill-gray">Teddy Barat</p>
                    </div>
                    <div class="role">
                        <div class="line-left"></div>
                        <h4 class="font h6">joueur</h4>
                        <div class="line-bottom"></div>
                    </div>
                    <div class="social">
                        <a href="https://twitter.com/samaa_meliodas">
                            <img src="/asset/img/icon/Twitter/white.png" alt="icon twitter">
                        </a>
                    </div>
                    <div class="player-item">
                        <img src="/asset/img/rocket-league/skins/car-2.png" alt="">
                    </div>
                    <div class="line-item"></div>
                </div>
            </div>
            <!-- /.col -->
            <div class="col-md-4 mb-5">
                <div class="card-player">
                    <div class="pseudo">
                        <h3 class="font h4 thrill-white">Ridley</h3>
                    </div>
                    <div class="name">
                        <p class="thrill-gray">Ridley Guery</p>
                    </div>
                    <div class="role">
                        <div class="line-left"></div>
                        <h4 class="font h6">joueur</h4>
                        <div class="line-bottom"></div>
                    </div>
                    <div class="social">
                        <a href="https://twitter.com/Ridleyistu">
                            <img src="/asset/img/icon/Twitter/white.png" alt="icon twitter">
                        </a>
                    </div>
                    <div class="player-item">
                        <img src="/asset/img/rocket-league/skins/car-1.png" alt="">
                    </div>
                    <div class="line-item"></div>
                </div>
            </div>
            <!-- /.col -->
        </div>
        <!-- /.row -->
    </div>
</section>

<?php require_once("../../includes/footer.php") ?>